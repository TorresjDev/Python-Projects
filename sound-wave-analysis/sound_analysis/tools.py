...
def detect_db_range(waveform):
    ...
    max_db = 20 * np.log10(max_amplitude /
                           1e-6) if max_amplitude > 0 else -np.inf
    min_db = 20 * np.log10(min_amplitude /
                           1e-6) if min_amplitude > 0 else -np.inf
        ...
        while True:
            try:
                ...
                if 1 <= choice_num <= len(wav_files):
                    ...
                    print(
                        f"❌ Please enter a number between 1 and {len(wav_files)}")
...
def get_analysis_options():
    ...
    options_list = [
        {"name": "Standard analysis (waveform + spectrogram)",
         "show_plots": True, "save_figures": False},
        {"name": "Save figures to files", "show_plots": True, "save_figures": True},
        {"name": "Analysis only (no plots)",
         "show_plots": False, "save_figures": False}
    ]
        ...
        while True:
            try:
                ...
                if event.event_type == keyboard.KEY_DOWN:
                    if event.name == 'up':
                        selected_index = (
                            selected_index - 1) % len(options_list)
                        display_options_menu()
                    elif event.name == 'down':
                        selected_index = (
                            selected_index + 1) % len(options_list)
