...
def perform_complete_analysis(file_path, show_plots=True, save_figures=False):
    ...
    try:
        ...
        print(f"🎧 Channels: {file_info['channels']} ({file_info['channel_type']})")
        ...
        print(f"📏 Dynamic Range: {audio_levels['db_range']['dynamic_range']:.2f} dB")
        ...
        if show_plots:
            ...
            if save_figures:
                ...
                waveform_path = os.path.join(figures_dir, f"{base_name}_waveform.png")
                spectrogram_path = os.path.join(figures_dir, f"{base_name}_spectrogram.png")
            else:
                waveform_path = None
