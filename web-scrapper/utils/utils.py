...
# New helper functions for user interaction
...
def select_files_to_download(file_links):
    ...
    for idx, (file_url, category) in enumerate(file_links, 1):
        print(f"{idx}. {file_url} (Category: {category})")
    ...
    while True:
        ...
        if choice == 'all':
            ...
            try:
                # Split the input into parts (e.g., "1 3-5 7")
                parts = choice.split()
                for part in parts:
                    if '-' in part:
    ...
    # Filter the file_links based on selected indices
